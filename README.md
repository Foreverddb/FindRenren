...
寻找恋之空是一个完全在浏览器中运行的静态前端应用。调色、文字与 Logo 合成、PNG 导出以及 MODNet/BEN2 人像抠图均在本地浏览器完成，不需要应用服务器、数据库、后端 API 或远程模型服务。

手机端使用内置 HSV 调色器，自定义颜色打开时饱和度与明度默认均为 100%；`Great Vibes` 与 `Ma Shan Zheng` 艺术字体也随站点静态托管，不依赖设备字体或外部 CDN。

高质量抠图会提示约 60–180 秒处理时间。模型尚未就绪时，页面继续复用进入页面后启动的静默下载，并显示已下载大小、总文件大小和实时百分比；模型下载不设超时，下载完成后会自动继续抠图。
...
构建产物约 258 MB，低于 GitHub Pages 发布站点 1 GB 限制。首次打开会按现有产品要求静默加载两套模型及 WASM，并加载约 6.3 MB 的本地艺术字体，冷缓存流量约 257 MB；GitHub Pages 的月度软带宽限制为 100 GB，因此适合个人展示和低流量使用，不适合作为高流量模型分发服务。

其他纯静态平台也能运行该应用，但托管方必须允许约 258 MB 的站点和 219 MB 的单个静态响应。若平台有更低的单文件上限，需要改用对象存储/CDN 托管 BEN2，或只部署低质量 MODNet，不能直接宣称完整兼容。
